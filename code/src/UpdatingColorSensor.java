...
public class UpdatingColorSensor extends ColorSensor {
	//refactor inheritance to delegation
	private final MyUpdatingSensor updatingSensor = new MyUpdatingSensor();
	/**
	...
	public void addListener(SensorListener listener) {
		//refactor inheritance to delegation
		UpdatingColorSensor.this.listeners.add(listener);
	}
	...
	private class MyUpdatingSensor implements UpdatingSensor {
		/** Updates the sensors state
         *
         */
        // refactor: Change signature
        public boolean updateState() {
            float now = (float) UpdatingColorSensor.this.getLightValue();
            if (now != value) {
                for (SensorListener s : listeners) {
                    s.stateChanged(this, value, now);
                }
            }
            UpdatingColorSensor.this.value = now;
            return true;
        }
	}
}
